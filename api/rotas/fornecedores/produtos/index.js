...
roteador.options('/:id', (requisicao, resposta) => {
  resposta.set('Access-Control-Allow-Methods', 'GET, DELETE, GET, HEAD, PUT')
  resposta.set('Access-Control-Allow-Headers', 'Content-Type')
  resposta.status(204)
  resposta.end()
})
...
roteador.delete('/:id', async (requisicao, resposta) => {
  const dados = {
    ...
    fornecedor: requisicao.idFornecedor.id
  }
  const produto = new Produto(dados);
...
roteador.get('/:id', async (requisicao, resposta, proximo) => {
  try{
    ...
    const serializador = new Serializador(resposta.getHeader('Content-Type'), ['preco', 'estoque', 'fornecedor', 'dataCriacao', 'dataAtualizacao', 'versao'])
    resposta.set('ETag', produto.versao)
    const timestamp = (new Date(produto.dataAtualizacao)).getTime()
    resposta.set('Last-Modified', timestamp)
    resposta.send(serializador.serializar(produto));
